...
import { FileSignature, Calendar, User, BookOpen, Loader2, CheckCircle, AlertCircle, Wallet, Lock, RefreshCw } from 'lucide-react';
...
const IssuerPortal: React.FC = () => {
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      if (error.code === 4001) {
        setConnectError("Access Denied: You rejected the wallet connection.");
      } else if (error.message && error.message.includes("No Wallet")) {
        setConnectError("Wallet Not Found: Please install MetaMask.");
      } else {
        setConnectError("Connection Failed: Make sure your wallet is unlocked.");
      }
    } finally {
  ...
  const handleMint = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await mintCredential(studentAddress, courseName, issueTimestamp, expirationTimestamp, file);
  ...
  if (!issuer) {
    return (
      <div className="max-w-2xl mx-auto flex flex-col items-center justify-center min-h-[60vh] text-center p-6">
        ...
        {connectError && (
          <div className="mb-6 p-4 bg-red-900/20 border border-red-500/50 rounded-lg flex items-center gap-3 text-left animate-in fade-in slide-in-from-bottom-2">
            <AlertCircle className="w-5 h-5 text-red-400 shrink-0" />
            <div className="flex-1">
              <p className="text-red-200 font-semibold text-sm">Authentication Failed</p>
              <p className="text-red-300/70 text-xs">{connectError}</p>
            </div>
            <button onClick={handleConnect} className="p-1 hover:bg-red-500/20 rounded text-red-300">
        ...
        <button
          ...
          className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-500 text-white px-8 py-4 rounded-xl font-bold text-lg transition-all shadow-lg shadow-indigo-900/20 disabled:opacity-70 disabled:cursor-not-allowed"
        >
